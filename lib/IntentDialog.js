...
})("IntentDialog", function moduleBody() {
...
IntentDialog["prototype"] = {
    ...
    "intent":       IntentDialog_intent,        // IntentDialog#intent(id, url):void
...
function IntentDialog_intent(id,    // @arg Integer
                             url) { // @arg URLString
                                    // @desc open intent url.
    var intentURL = _createIntentURL(url, this._package[id]);

    if (IntentDialog["VERBOSE"]) {
        alert("Open Chrome Intent URL:\n" + intentURL);
    }
...
function _localization(fragment, lang) {
    ...
    for (var key in resource) {
        ...
        fragment = fragment.replace(rex, fn);
    }
    return fragment;
}
